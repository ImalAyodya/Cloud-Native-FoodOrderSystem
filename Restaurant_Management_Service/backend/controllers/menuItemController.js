...
const getMenuItems = async (req, res) => {
  try {
    ...
    // Build filter object - but DON'T include restaurantId filter
    const filter = {};
    ...
    // Set up pagination - use a higher limit to show more items at once
    const pageNum = parseInt(page) || 1;
    const limit = parseInt(requestedLimit) || 100; // Use larger default limit
    const skip = (pageNum - 1) * limit;

    // Find menu items with pagination but higher limit
    const menuItems = await MenuItem.find(filter)
      .populate('restaurantId', 'name images.logo priceRange rating location.city')
